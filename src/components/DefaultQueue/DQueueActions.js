...
export function getDefaultSongs(queueId) {
	return {
		...
		payload: axios
			.get(`/api/Queues/${queueId}/default`)
...
export function addDefaultSong(inputValue, queueId) {
	return {
		...
		payload: axios
			.post(`/api/Queues/addToDefaultSongs`, { id: queueId, uri: inputValue })
			.then(response => response.data.defaultSongs),
	};
}
